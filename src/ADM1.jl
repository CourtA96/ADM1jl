...
# Examples
```jldoctest
julia> u0 = ADM1.InitialConditions();

julia> rp = ADM1.reactorParameterDefinition();

julia> php = ADM1.physiochemicalParameterDefinition(rp);

julia> ADM1.pressureOfGasses(u0,php,rp)
...
# Examples
```jldoctest
julia> ADM1.monod(3.0,2.0) # when u is non-zero positive
0.6
```
```jldoctest
julia> ADM1.monod(-3.0,2.0) # when u is negative
0
```
...
julia> bp = ADM1.biochemicalparameter_definition();

julia> rp = ADM1.reactorParameterDefinition();

julia> php = ADM1.physiochemicalParameterDefinition(rp);

julia> pressures = ADM1.pressureOfGasses(u0,php,rp);

julia> NREAC = 29;

julia> ADM1.reactionrates(bp,rp,php,pressures,u0,NREAC)
...
julia> IV = ADM1.inflowvector_definition();

julia> sol, tSol = ADM1.ExampleSol((0.0,200.0),u0,IV);
...
julia> sol, tSol = ADM1.ADM1sol((0.0,200.0),u0,IV);
...
julia> IV_temp = ADM1.inflowvector_definition();

julia> IV = [IV_temp*(0.5*rand()+1.0) for i in 1:length(t)]

julia> sol,tSol = ADM1.ADM1sol((0.0,50.0),u0,IV,t);
